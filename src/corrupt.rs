...
impl Corruptor {
    ...
    fn blast(&mut self, mem: &mut Mem) {
        ...
        let total_bytes: u64 = allocations.iter().map(|&(_, size)| size as u64).sum();
        if total_bytes == 0 {
            return;
        }
...
mod tests {
    ...
    fn corrupts_only_live_allocation() {
        ...
        assert!(
            corruptor.total_corrupted() > 0,
            "engine reported no corruption"
        );
        assert!(
            changed > 0,
            "no bytes were actually changed in guest memory"
        );
    }
    ...
    fn disabled_engine_does_nothing() {
        ...
        assert!(
            ...
            "disabled engine corrupted memory"
        );
    ...
    fn interval_gates_bursts() {
        ...
        assert_eq!(
            corruptor.total_corrupted(),
            0,
            "burst fired before interval elapsed"
        );
        // 10th tick fires.
        corruptor.tick(&mut mem);
        assert!(
            corruptor.total_corrupted() > 0,
            "burst did not fire at interval"
        );
    }
